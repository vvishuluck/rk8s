//! 集成测试：生成 nftables 规则并用 nft --check 校验
...
fn diagnose_transport_payloads(json_str: &str) {
    if let Ok(val) = serde_json::from_str::<serde_json::Value>(json_str) {
        if let Some(arr) = val.get("nftables").and_then(|v| v.as_array()) {
            for (idx, obj) in arr.iter().enumerate() {
                if let Some(rule) = obj.get("rule") {
                    if let Some(expr_arr) = rule.get("expr").and_then(|v| v.as_array()) {
                            ...
                            if let Some(m) = expr.get("match") {
                                if let Some(left) = m.get("left") {
                                    if let Some(payload) = left.get("payload") {
                                        if let Some(field) = payload.get("field") {
                                            if let Some(proto) = payload.get("protocol").and_then(|v| v.as_str()) {
                                                if proto == "tcp" || proto == "udp" {
                                                    has_transport_payload = true;
                                                    transport_protocol = proto.to_string();
                                                    if !has_l4proto_match {
                                                        println!("\u26a0\ufe0f  Rule #{} expr[{}]: Found {} payload without prior l4proto match", idx, expr_idx, proto);
                        ...
                        if has_transport_payload && !has_l4proto_match {
                            println!("\u274c Rule #{}: Uses {} payload WITHOUT l4proto match", idx, transport_protocol);
